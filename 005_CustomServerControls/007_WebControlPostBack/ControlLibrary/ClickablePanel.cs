...
namespace ControlLibrary
{
    public class ClickablePanel : Panel, IPostBackEventHandler
    {
        protected override void AddAttributesToRender(HtmlTextWriter writer)
        {
            ...
            // Создает на странице JavaScript функцию __doPostBack() для Postback запроса (если такого скрипта там нет).
            // Также, этот метод, устанавливает в качестве обработчика нажатия по кнопке вызов метода _doPostBack
            // с передачей в него своего ID и параметров для события (первый и второй аргумент метода GetPostBackEventReference соответственно)
